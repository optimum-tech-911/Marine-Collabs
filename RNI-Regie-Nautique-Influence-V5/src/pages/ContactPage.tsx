import { ArrowRight, CalendarDays, CheckCircle2, Mail, MessageSquareText, UserRoundSearch } from 'lucide-react';
...
const routes = [
  ...
  { id: 'rendez-vous', label: 'Prendre rendez-vous', icon: CalendarDays },
  { id: 'createur', label: 'Candidature créateur', icon: UserRoundSearch },
];

export function ContactPage() {
  const [params] = useSearchParams();
  const initialIntent = params.get('intent') === 'rendez-vous' ? 'rendez-vous' : params.get('intent') === 'createur' ? 'createur' : 'marque';
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const body = [
      ...
      ...fields,
      '',
      'Demande préparée depuis la plateforme Régie Nautique d’Influence.',
    ].filter(Boolean).join('\n');
    ...
    <>
      ...
      <section className="section contact-v4"><div className="container contact-v4__grid"><SectionReveal className="contact-v4__routes"><p className="eyebrow">VOTRE PARCOURS</p><h2>Pourquoi contactez-vous Régie Nautique ?</h2><div>{routes.map(({id,label,icon:Icon},index)=><button className={path===id?'active':''} type="button" key={id} onClick={()=>{setPath(id);setSubmitted(false);}}><span>{String(index+1).padStart(2,'0')}</span><Icon size={19}/><strong>{label}</strong><ArrowRight size={17}/></button>)}</div><aside><strong>Un vrai interlocuteur</strong><p>Pas de bot. Pas de script. Les demandes sont relues avant toute proposition.</p></aside></SectionReveal>
        ...
        <SectionReveal className="contact-v4__panel" delay={.06}>{submitted ? <div className="success-state"><CheckCircle2 size={42}/><h2>Votre demande est prête.</h2><p>Cette version statique ne transmet pas encore automatiquement le formulaire. Continuez par email pour envoyer les informations préparées.</p><a className="button button--dark" href={preparedMailto}>Continuer par email <ArrowRight size={17}/></a></div> : <form onSubmit={submit}><div className="form-heading"><p className="eyebrow">{routes.find((item)=>item.id===path)?.label.toUpperCase()}</p><h2>{path==='rendez-vous'?'Choisissez le contexte de l’échange.':'Transmettez les informations essentielles.'}</h2></div><div className="form-grid">
          ...
          {path==='marque'?<><label><span>Fourchette de budget</span><select required name="budget" defaultValue=""><option value="" disabled>Sélectionner</option><option>Moins de 5 000 €</option><option>5 000 €–15 000 €</option><option>15 000 €–50 000 €</option><option>50 000 €+</option></select></label><label><span>Calendrier visé</span><input required name="timing" placeholder="Ex. lancement en octobre 2026"/></label></>:null}
          {path==='rendez-vous'?<><label><span>Secteur</span><input name="sector" placeholder="Ex. chantier naval, équipementier…"/></label><label><span>Disponibilités</span><input name="availability" placeholder="Ex. mardi ou jeudi après-midi"/></label></>:null}
