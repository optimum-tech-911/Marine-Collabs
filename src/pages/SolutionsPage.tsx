...
export function SolutionsPage() {
  return (
    <>
      <section className="solutions-v4-hero"><div className="solutions-v4-hero__grid" aria-hidden="true"/><div className="container solutions-v4-hero__inner"><div><p className="eyebrow eyebrow--light">SOLUTIONS DE CAMPAGNE</p><h1>Commencez par l’objectif. Nous construisons le dispositif.</h1><p>Le casting, les formats, les droits et la mesure doivent découler du résultat attendu — pas d’une liste de livrables copiée d’une campagne précédente.</p><div className="solutions-v4-hero__actions"><ContactLink className="button button--primary button--hero" placement="solutions_hero">Parler à Adrien <ArrowRight size={18}/></ContactLink><ContactLink className="button button--ghost-light button--hero" placement="solutions_hero_secondary">Échanger sur votre projet</ContactLink></div></div><div className="solutions-v4-hero__visual"><img src="/assets/brand/fieldwork/navigation-equipment.jpg" alt="Navigateur utilisant un équipement électronique marin à la barre"/><span><strong>06</strong><small>formats de campagne structurés</small></span></div></div></section>

      <section className="section solutions-v4-list"><div className="container"><SectionReveal className="solutions-v4-list__heading"><p className="eyebrow">OBJECTIFS</p><h2>Six points de départ, autant de castings possibles.</h2></SectionReveal><div className="solutions-v4-grid">{solutions.map(({icon:Icon,title,text,deliverables},index)=><SectionReveal className="solutions-v4-card" delay={index*.04} key={title}><span className="solutions-v4-card__index">{String(index+1).padStart(2,'0')}</span><Icon size={24}/><h3>{title}</h3><p>{text}</p><ul>{deliverables.map((item)=><li key={item}>{item}</li>)}</ul><ContactLink placement={`solution_${index + 1}`}>Échanger sur ce format <ArrowRight size={16}/></ContactLink></SectionReveal>)}</div></div></section>
      ...
      <section className="solutions-v4-cta"><div className="container solutions-v4-cta__inner"><div><p className="eyebrow">UN PROJET EN TÊTE ?</p><h2>Transformez une idée encore floue en échange concret.</h2></div><ContactLink className="button button--dark button--hero" placement="solutions_footer">Parler à Adrien <ArrowRight size={18}/></ContactLink></div></section>
    </>
  );
}
