  ...
  return <footer className="site-footer">
    <div className="container footer-main">
      ...
      <div className="footer-links">
        ...
        <div><p className="footer-label">{fr ? 'Agence' : 'Agency'}</p><Link to={path('about')}>{fr ? 'À propos' : 'About'}</Link><Link to={path('methodology')}>{fr ? 'Méthodologie & sources' : 'Methodology & sources'}</Link><Link to={path('contact')}>Contact</Link><a href={`mailto:${brand.email}`}>{brand.email}</a><span>{brand.location}</span></div>
        <div><p className="footer-label">{fr ? 'Réseaux & contact' : 'Social & contact'}</p>{brand.socials.instagram ? <a href={brand.socials.instagram} target="_blank" rel="noreferrer"><Camera size={16}/> Instagram</a> : null}{brand.socials.linkedin ? <a href={brand.socials.linkedin} target="_blank" rel="noreferrer"><BriefcaseBusiness size={16}/> LinkedIn</a> : null}<Link to={`${path('contact')}?intent=rendez-vous`}>{fr ? 'Prendre rendez-vous' : 'Book a call'}</Link><Link to={path('campaign')}>{fr ? 'Préparer un brief' : 'Prepare a brief'}</Link><LanguageSwitcher /></div>
      </div>
    </div>
