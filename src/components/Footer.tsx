...
export function Footer() {
  ...
  return <footer className="site-footer">
    <div className="container footer-main">
      <div className="footer-intro"><Logo inverse/><h2>{fr ? 'Placez votre marque là où la culture nautique se vit réellement.' : 'Place your brand where maritime culture is genuinely lived.'}</h2><ContactLink className="button button--light" placement="footer_contact">{fr ? 'Parler à Adrien' : 'Talk to Adrien'} <ArrowUpRight size={17}/></ContactLink></div>
      <div className="footer-links">
        ...
        <div><p className="footer-label">{fr ? 'Réseaux & contact' : 'Social & contact'}</p>{brand.socials.instagram ? <a href={brand.socials.instagram} target="_blank" rel="noreferrer"><Camera size={16}/> Instagram</a> : null}{brand.socials.linkedin ? <a href={brand.socials.linkedin} target="_blank" rel="noreferrer"><BriefcaseBusiness size={16}/> LinkedIn</a> : null}<a href={brand.whatsappUrl} target="_blank" rel="noreferrer">{fr ? 'Parler à Adrien' : 'Contact Adrien'} · {brand.phone}</a><a href={`mailto:${brand.email}`}>{brand.email}</a><LanguageSwitcher /></div>
      </div>
    </div>
