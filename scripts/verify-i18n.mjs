...
const englishCopy = read('src/i18n/english-copy.ts');
...
for (const token of [
  'A network mapped at the right level of precision.',
  'A maritime campaign is lived before it is published.',
  'No profile matches.',
  'DETAILED COMPARISON',
  'KREW MEDIA — CAMPAIGN BRIEF',
  'Profile evidence · captured 15 July 2026',
]) if (!englishCopy.includes(token)) errors.push(`English interface copy is missing: ${token}`);
